...
def decayAlpha(initialValue, finalValue, maxSteps, decayType):
    ...
    if decayType=="Exp":
        ...
        for i in range(maxSteps,1000):
            step_size[i] = 0.01

    return step_size

alpha=(decayAlpha(0.75,0.01,750,"Exp"))/500
# print(alpha)
...
epsilon=(decayEps(1,0.01,750,"Exp"))
# print(epsilon)
...
def double_q_learning(env, gamma, alpha, epsilon, noEpisodes):
    # your code goes here
    Q1 = np.zeros((25,2))
    Qs1 = np.zeros((noEpisodes,25,2))
    Q2 = np.zeros((25,2))
    Qs2 = np.zeros((noEpisodes,25,2))
    Q = np.zeros((25,2))
    Qs = np.zeros((noEpisodes,25,2))
    Q3 = np.zeros((25,2))
    Qs3 = np.zeros((noEpisodes,25,2))
    Vs = np.zeros((noEpisodes,25))
    Ps = np.zeros((noEpisodes,25))
    ...
    for e in range(noEpisodes):
        ...
        while not done:
            ...
            if choose == 1:
                ...
                td_target = r
                if not done:
                    td_target = td_target + gamma*(Q2[sprime][aq1]+Q3[sprime][aq1])/2
                td_error = td_target - Q1[s][a]
                Q1[s][a] = Q1[s][a] + alpha0*td_error
            elif choose == 2:
                aq3 = np.argmax(Q3[sprime])
                td_target = r
                if not done:
                    td_target = td_target + gamma*(Q2[sprime][aq3]+Q1[sprime][aq3])/2
                td_error = td_target - Q3[s][a]
                Q3[s][a] = Q3[s][a] + alpha0*td_error
            else:
                aq2 = np.argmax(Q2[sprime])
                td_target = r
                if not done:
                    td_target = td_target + gamma*(Q1[sprime][aq2]+Q3[sprime][aq2])/2
...
for i in range(1):
    ...
    r2,a2,q2=double_q_learning(env2,1,alpha,epsilon,1000)
    r3,a3,q3=double_q_learning(env3,1,alpha,epsilon,1000)
    r4,a4,q4=double_q_learning(env4,1,alpha,epsilon,1000)
...
# plt.xlabel("Episodes")
# plt.ylabel("Rewards")
# plt.title("Triple Q learning Rewards vs Episodes")
