    ...
    def step(self, action: int):
        ...
        if action == 1:
            if self.elapsed_time + decision_time > self.total_time:
                ...
                reward = self.reward_mean - 0.5*self.count + np.random.normal(0,0.025,1)
            ...
            if self.elapsed_time + self.interval_time > self.total_time:
                ...
                self.count = 0
                self.reward_mean = np.random.choice([np.random.randint(2,7), np.random.randint(10,15)])
